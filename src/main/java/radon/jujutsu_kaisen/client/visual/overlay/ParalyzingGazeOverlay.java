...
public class ParalyzingGazeOverlay implements IOverlay {
    private static final RenderType PARALYZING_GAZE = JJKRenderTypes.eyes(new ResourceLocation(JujutsuKaisen.MOD_ID, "textures/overlay/paralyzing_gaze.png"));
    @Override
    public boolean isValid(LivingEntity entity, ClientVisualHandler.ClientData data) {
        return data.toggled.contains(JJKAbilities.PARALYZING_GAZE.get());
    }
    ...
    public <T extends LivingEntity> void render(T entity, ClientVisualHandler.ClientData data, ResourceLocation texture, EntityModel<T> model, PoseStack poseStack, MultiBufferSource buffer, float partialTicks, int packedLight) {
        VertexConsumer consumer = buffer.getBuffer(PARALYZING_GAZE);
